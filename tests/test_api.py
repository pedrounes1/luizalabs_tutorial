...
class TestListOrders:
    ...
    def test_obter_itens_quando_identificacao_do_pedido_nao_encontrado_um_erro_deve_ser_retornado(self, client, overrides_get_items_by_order):
    ...
    def test_found_should_return_ok_code(self, client, overrides_get_items_by_order):
        overrides_get_items_by_order([])
        response = client.get("/orders/7e290683-d67b-4f96-a940-44bef1f69d21/items")
        assert response.status_code == HTTPStatus.OK

    def test_found_shoud_return_items(self, client, overrides_get_items_by_order):
        items = [
