...
def handle_communication_fail(request: Request, exc: CommunicationFailError):
    return JSONResponse(
        ...
        content={"message": "Communication with server failed"},
    )
...
@app.get(
    ...
    description="Verify if server is on",
...
@app.get(
    ...
    tags=["orders"],
    summary="Items of an order",
    description="Return all items of a given order",
